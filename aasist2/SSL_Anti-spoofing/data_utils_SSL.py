...
class Dataset_ASVspoof2019_train(Dataset):
    ...
    def __getitem__(self, index):
        ...
        X, fs = sf.read(self.base_dir + utt_id)
...
class Dataset_Eval_eval(Dataset):
    def __init__(self, database_pase, protocols_file):
        """self.list_IDs	: list of strings (each string: utt key),"""

        self.database_pase = database_pase
        ...
        labels = []
        for line in lines:
            variant, file, random_token, src, label = line.strip().split(" ")
    ...
    def __getitem__(self, index):
        ...
        utt_id = self.files[index]
        X, fs = librosa.load(os.path.join(self.database_pase, utt_id), sr=None)
        X_pad = pad(X, self.cut)
        x_inp = Tensor(X_pad)
        variant = self.variants[index]
        random_token = self.random_tokens[index]
        src = self.srcs[index]
        label = self.labels[index]
        return x_inp, variant, utt_id, random_token, src, label
