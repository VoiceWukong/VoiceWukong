...
def evaluate(test_loader, model, comment):
    ...
    with torch.no_grad():
        ...
        for step, (input,variant, file_name,random_token, attack_id, target) in tqdm(enumerate(test_loader)):
            ...
            # eermeter.update(target, output)
            # total_acc_meter.update(target, output)
            variant_list.extend(list(variant))
            fname_list.extend(list(file_name))
            key_list.extend(
            ['bonafide' if key == 1 else 'spoof' for key in target.tolist()])
            random_toekn_list.extend(list(random_token))
            
            att_id_list.extend(list(attack_id))
            score_list.extend(output[:,1].tolist())
        # end_time=time.time()
        # print(f'{comment} use time : {end_time-start_time} s')
    save_path =  comment + '_eval_score.txt'
    with open(save_path, 'a') as fh:
        for v,f,r, s, k, cm in zip(variant_list,fname_list,random_toekn_list, att_id_list, key_list, score_list):
            fh.write('{} {} {} {} {} {}\n'.format(v,f,r, s, k, cm))