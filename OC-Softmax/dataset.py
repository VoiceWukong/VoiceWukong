...
class ASVspoof2019(Dataset):
    ...
    def __getitem__(self, idx):
        ...
        variant, filename, flag, src, label = self.all_info[idx]
        # try:
            # with open(self.ptf + '/'+ filename + self.feature + '.pkl', 'rb') as feature_handle:
            #     feat_mat = pickle.load(feature_handle)
        with open(self.ptf + '/LFCC_'+ filename+ '.pkl', 'rb') as feature_handle:
            feat_mat = pickle.load(feature_handle)
        # except:
        ...
        return feat_mat, variant,filename, flag,src, self.label[label]
