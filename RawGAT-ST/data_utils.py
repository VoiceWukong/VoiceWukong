...
ASVFile = collections.namedtuple('ASVFile',
    ['variant', 'file_name', 'random', 'src', 'key'])
...
class ASVDataset(Dataset):
    ...
    def __init__(self, database_path=None,protocols_path=None,transform=None, 
        ...
        self.files_dir = os.path.join(self.data_root_dir)
        ...
        # self.cache_fname = f'{protocols_path}.npy'
        # print('cache_fname',self.cache_fname)
        
        
        self.transform = transform
        self.count=0
        self.files_meta = self.parse_protocols_file(self.protocols_dir)
        # if os.path.exists(self.cache_fname):
        #     self.data_x, self.data_y, self.data_sysid, self.files_meta = torch.load(self.cache_fname)
        #     print('Dataset loaded from cache ', self.cache_fname)
        # if os.path.exists(self.cache_fname):
        #     self.data_x, self.data_y, self.variant,self.file_name,self.random,self.src,self.files_meta = torch.load(self.cache_fname)
        #     print('Dataset loaded from cache ', self.cache_fname)
        # # else:
        ...
        #     self.data_x, self.data_y, self.variant,self.file_name,self.random,self.src = map(list, zip(*data))
        #     torch.save((self.data_x, self.data_y, self.variant,self.file_name,self.random,self.src,self.files_meta), self.cache_fname)
    ...
    def __getitem__(self, idx):
        x,y,_,_,_,_=self.read_file(self.files_meta[idx])
        return x, y, self.files_meta[idx]

    def read_file(self, meta):
        
        # data_x, sample_rate = sf.read(meta.path)
        data_x, sample_rate = sf.read(os.path.join(self.data_root_dir,meta.file_name))
        data_y = meta.key
        ...
        return data_x, float(data_y), meta.variant,meta.file_name,meta.random,meta.src
    ...
    def _parse_line(self, line):
        ...
        #     sys_id=self.sysid_dict[tokens[3]],
        #     key=int(tokens[4] == 'bonafide'))
        return ASVFile(variant=tokens[0],
            file_name=tokens[1],
            # path=os.path.join(self.files_dir, tokens[1] + '.flac'),
            random=tokens[2],
            # sys_id=self.sysid_dict[tokens[3]],
