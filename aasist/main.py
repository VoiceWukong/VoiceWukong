...
def main(args: argparse.Namespace) -> None:
    ...
    """
    "model_config": {
        "architecture": "AASIST",模型架构
        "nb_samp": 64600,样本数量
        "first_conv": 128,卷积层大小
        "filts": [70, [1, 32], [32, 32], [32, 64], [64, 64]],卷积层过滤器大小
        "gat_dims": [64, 32],维度 包含GAT每层的维度
        "pool_ratios": [0.5, 0.7, 0.5, 0.5], 池化比例
        "temperatures": [2.0, 2.0, 100.0, 100.0] 注意力层温度参数
    },
    """
    model_config = config["model_config"]
    """ "optim_config": {
        "optimizer": "adam", 优化器
        "amsgrad": "False",是否使用amsgrad版本的优化器
        "base_lr": 0.0001,基础学习率
        "lr_min": 0.000005,最小学习率
        "betas": [0.9, 0.999],优化器的两个超参数
        "weight_decay": 0.0001,权重衰减系数
        "scheduler": "cosine"学习率调度类型
    }"""
    optim_config = config["optim_config"]
    """训练轮数"""
    optim_config["epochs"] = config["num_epochs"]
    track = config["track"]
    """断言语句，检查track是否在["LA", "PA", "DF"]中，否则抛出异常"""
    assert track in ["LA", "PA", "DF"], "Invalid track given"
    if "eval_all_best" not in config:
        """对训练过程中所有轮数的最佳模型进行评估 而不是只评估最后一轮的最佳模型"""
        config["eval_all_best"] = "True"
    if "freq_aug" not in config:
        config["freq_aug"] = "False"

    # make experiment reproducible
    """为实验中的随机数生成器设置种子  并且按照config文件配置cudnn 使得实验结果可以复现"""
    ...
    if args.eval:
        ...
        """需要改写eval_loader eval_trial_path的构建"""
